// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from synapse_msgs:msg/WarehouseShelf.idl
// generated code does not contain a copyright notice
#include "synapse_msgs/msg/detail/warehouse_shelf__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `object_name`
// Member `qr_decoded`
#include "rosidl_runtime_c/string_functions.h"
// Member `object_count`
#include "rosidl_runtime_c/primitives_sequence_functions.h"

bool
synapse_msgs__msg__WarehouseShelf__init(synapse_msgs__msg__WarehouseShelf * msg)
{
  if (!msg) {
    return false;
  }
  // object_name
  if (!rosidl_runtime_c__String__Sequence__init(&msg->object_name, 0)) {
    synapse_msgs__msg__WarehouseShelf__fini(msg);
    return false;
  }
  // object_count
  if (!rosidl_runtime_c__uint8__Sequence__init(&msg->object_count, 0)) {
    synapse_msgs__msg__WarehouseShelf__fini(msg);
    return false;
  }
  // qr_decoded
  if (!rosidl_runtime_c__String__init(&msg->qr_decoded)) {
    synapse_msgs__msg__WarehouseShelf__fini(msg);
    return false;
  }
  return true;
}

void
synapse_msgs__msg__WarehouseShelf__fini(synapse_msgs__msg__WarehouseShelf * msg)
{
  if (!msg) {
    return;
  }
  // object_name
  rosidl_runtime_c__String__Sequence__fini(&msg->object_name);
  // object_count
  rosidl_runtime_c__uint8__Sequence__fini(&msg->object_count);
  // qr_decoded
  rosidl_runtime_c__String__fini(&msg->qr_decoded);
}

bool
synapse_msgs__msg__WarehouseShelf__are_equal(const synapse_msgs__msg__WarehouseShelf * lhs, const synapse_msgs__msg__WarehouseShelf * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // object_name
  if (!rosidl_runtime_c__String__Sequence__are_equal(
      &(lhs->object_name), &(rhs->object_name)))
  {
    return false;
  }
  // object_count
  if (!rosidl_runtime_c__uint8__Sequence__are_equal(
      &(lhs->object_count), &(rhs->object_count)))
  {
    return false;
  }
  // qr_decoded
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->qr_decoded), &(rhs->qr_decoded)))
  {
    return false;
  }
  return true;
}

bool
synapse_msgs__msg__WarehouseShelf__copy(
  const synapse_msgs__msg__WarehouseShelf * input,
  synapse_msgs__msg__WarehouseShelf * output)
{
  if (!input || !output) {
    return false;
  }
  // object_name
  if (!rosidl_runtime_c__String__Sequence__copy(
      &(input->object_name), &(output->object_name)))
  {
    return false;
  }
  // object_count
  if (!rosidl_runtime_c__uint8__Sequence__copy(
      &(input->object_count), &(output->object_count)))
  {
    return false;
  }
  // qr_decoded
  if (!rosidl_runtime_c__String__copy(
      &(input->qr_decoded), &(output->qr_decoded)))
  {
    return false;
  }
  return true;
}

synapse_msgs__msg__WarehouseShelf *
synapse_msgs__msg__WarehouseShelf__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  synapse_msgs__msg__WarehouseShelf * msg = (synapse_msgs__msg__WarehouseShelf *)allocator.allocate(sizeof(synapse_msgs__msg__WarehouseShelf), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(synapse_msgs__msg__WarehouseShelf));
  bool success = synapse_msgs__msg__WarehouseShelf__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
synapse_msgs__msg__WarehouseShelf__destroy(synapse_msgs__msg__WarehouseShelf * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    synapse_msgs__msg__WarehouseShelf__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
synapse_msgs__msg__WarehouseShelf__Sequence__init(synapse_msgs__msg__WarehouseShelf__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  synapse_msgs__msg__WarehouseShelf * data = NULL;

  if (size) {
    data = (synapse_msgs__msg__WarehouseShelf *)allocator.zero_allocate(size, sizeof(synapse_msgs__msg__WarehouseShelf), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = synapse_msgs__msg__WarehouseShelf__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        synapse_msgs__msg__WarehouseShelf__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
synapse_msgs__msg__WarehouseShelf__Sequence__fini(synapse_msgs__msg__WarehouseShelf__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      synapse_msgs__msg__WarehouseShelf__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

synapse_msgs__msg__WarehouseShelf__Sequence *
synapse_msgs__msg__WarehouseShelf__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  synapse_msgs__msg__WarehouseShelf__Sequence * array = (synapse_msgs__msg__WarehouseShelf__Sequence *)allocator.allocate(sizeof(synapse_msgs__msg__WarehouseShelf__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = synapse_msgs__msg__WarehouseShelf__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
synapse_msgs__msg__WarehouseShelf__Sequence__destroy(synapse_msgs__msg__WarehouseShelf__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    synapse_msgs__msg__WarehouseShelf__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
synapse_msgs__msg__WarehouseShelf__Sequence__are_equal(const synapse_msgs__msg__WarehouseShelf__Sequence * lhs, const synapse_msgs__msg__WarehouseShelf__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!synapse_msgs__msg__WarehouseShelf__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
synapse_msgs__msg__WarehouseShelf__Sequence__copy(
  const synapse_msgs__msg__WarehouseShelf__Sequence * input,
  synapse_msgs__msg__WarehouseShelf__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(synapse_msgs__msg__WarehouseShelf);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    synapse_msgs__msg__WarehouseShelf * data =
      (synapse_msgs__msg__WarehouseShelf *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!synapse_msgs__msg__WarehouseShelf__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          synapse_msgs__msg__WarehouseShelf__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!synapse_msgs__msg__WarehouseShelf__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
